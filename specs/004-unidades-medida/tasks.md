...
- [X] T005 Crear `loopi-api-v2/db/migrations/NNNN_crear_tabla_unidades_medida.up.sql` con DDL
...
- [X] T006 [P] Crear `loopi-api-v2/db/migrations/NNNN_crear_tabla_unidades_medida.down.sql`
  con `DROP TABLE IF EXISTS unidades_medida`
- [X] T007 [P] Crear `loopi-api-v2/db/migrations/NNNN+1_seed_unidades_medida.up.sql` con INSERT
...
- [X] T008 [P] Crear `loopi-api-v2/db/migrations/NNNN+1_seed_unidades_medida.down.sql` con
  `DELETE FROM unidades_medida WHERE codigo IN ('g','ml','und','kg','t','mg','L','dL','cL','docena','par','caja')`
- [X] T009 Aplicar migraciones: `migrate -path ./db/migrations -database "$DB_DSN" up 2`;
...
- [X] T010 Crear `loopi-api-v2/internal/unidades_medida/model.go` con structs Go:
...
- [X] T011 [P] Crear `loopi-api-v2/internal/unidades_medida/cache.go`: inicializar instancia
...
- [X] T013 Crear `loopi-api-v2/internal/conversion/conversion.go` con función pura
...
- [X] T014 [P] Crear `loopi-api-v2/internal/conversion/conversion_test.go` con casos:
...
- [X] T015 Ejecutar `go test ./internal/conversion/... -v -coverprofile=coverage.out` desde
...
- [X] T016 Crear `loopi-api-v2/internal/unidades_medida/repository.go` con interfaz
...
- [X] T017 Crear `loopi-api-v2/internal/unidades_medida/service.go` con struct `UMService`
...
- [X] T018 [P] Agregar `Inactivar(ctx, id int64) error` a `service.go`: obtener unidad (404
...
- [X] T019 [P] Agregar `ObtenerImpacto(ctx, id int64) (*ImpactoResponse, error)` a `service.go`:
...
- [X] T020 Crear `loopi-api-v2/internal/unidades_medida/handler.go` con struct `UMHandler`
...
- [X] T021 [P] Agregar `InactivarUnidad(w, r)` y `ObtenerImpacto(w, r)` a `handler.go`:
...
- [X] T022 Registrar en el router de `loopi-api-v2`:
...
- [X] T023 [P] Crear `loopi-api-v2/internal/unidades_medida/service_test.go` con mock de
...
- [X] T023b [P] Crear `loopi-api-v2/internal/unidades_medida/repository_test.go` con
...
- [X] T029 Agregar a `loopi-api-v2/internal/unidades_medida/repository.go`:
...
- [X] T030 [P] Agregar a `loopi-api-v2/internal/unidades_medida/service.go`:
...
- [X] T031 Agregar `ListarUnidades(w, r)` a `handler.go`: parsear query params `tipo`, `activo`,
...
- [X] T032 [P] Agregar `ObtenerUnidad(w, r)` a `handler.go`: parsear `{id}` del path; llamar
  `service.ObtenerPorID`; retornar 200 `DetalleUMResponse`; 404 con `unidad_no_encontrada`
  si no existe
- [X] T033 Agregar al router: `GET /api/v1/unidades_medida` con middleware `autenticacion` (todos
  los roles), `GET /api/v1/unidades_medida/{id}` con middleware `autenticacion` (todos los roles)

### Backend — Tests HU3

- [X] T034 [P] Agregar a `service_test.go`:
...
- [X] T040 Agregar `Editar(ctx, id int64, req *EditarUMRequest) (*UnidadMedida, error)` a
...
- [X] T041 Agregar `Editar(ctx, id int64, req *EditarUMRequest) (*UnidadMedida, error)` a
...
- [X] T042 [P] Agregar `EditarUnidad(w, r)` a `handler.go`: parsear body `EditarUMRequest`;
...
- [X] T043 [P] Agregar `PUT /api/v1/unidades_medida/{id}` al router con middlewares
  `autenticacion` y `solo_admin`

### Backend — Tests HU2

- [X] T044 [P] Agregar a `service_test.go`:
...
- [X] T046 [P] Agregar `editarUnidad(id: number, req: EditarUnidadMedidaRequest): Observable<UnidadMedida>`
...
- [X] T047 [P] Verificar y completar trazas OpenTelemetry en
...
- [X] T048 [P] Verificar y completar logs JSON estructurados en
...
- [X] T051 [P] Ejecutar gates de backend en `loopi-api-v2/`:
...
- [X] T052 [P] Ejecutar gates de frontend en `loopi-web-v2/`:
