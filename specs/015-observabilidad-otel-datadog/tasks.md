...
**Prueba independiente**: `curl` externo al agente retorna 403 o conexión rechazada; `gitleaks` no detecta secrets.

- [ ] T024 [P] [US4] Verificar que el agente en stage rechaza tráfico externo: `curl -v https://dd-agent-<HASH>-uc.a.run.app` desde fuera de GCP debe retornar error de acceso (403 o conexión rechazada por `--ingress=internal`)
...
- [ ] T026 [P] [US4] Verificar que `app.yaml`, `app.stage.yaml` y `app.prod.yaml` no contienen el valor real de `DD_API_KEY`: `grep -r "DD_API" loopi-api-v2/app*.yaml` debe mostrar solo referencias a Secret Manager, no el valor
...
- T010 depende de T009 (necesita la URL del agente para las modificaciones en `main.go`)
- T011, T012 y T013 son independientes de T010 [P] — pueden editarse en paralelo con T010
- T013 depende de T009 (necesita la URL de stage para `app.stage.yaml`)
...
T011: app.yaml
T012: app.prod.yaml
T013: app.stage.yaml  ← paralela con T011 y T012; depende de T009
...
- Las URLs de Cloud Run se obtienen en T009 y se usan en T012, T013 y T015
...
- `--ingress=internal` sin `--no-allow-unauthenticated`: el aislamiento es por red GCP, no por IAM; el exporter OTLP no necesita tokens
